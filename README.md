...
assigns the card to itself. `turn_timeout` stops a turn
...
The daemon deletes a log that no turn wrote to for `log_retention` (30 days
by default; "0" keeps the logs forever). The card state, with the session and
the costs, stays.
